...
**Real-world artifact to check:** The shadow mask (or aperture grille) of a consumer Trinitron or slotted-mask CRT at standard viewing distance. On a real 13–20 inch consumer TV of the 1980s–90s, individual phosphor triads are visible at close range but merge at 1m. The gap/mask geometry differs by CRT type: shadow-mask and slotted-mask displays use a triad dot or slot grid with horizontal dark bands between rows of phosphors (the bands are narrower in the centre and widen slightly toward the edges due to beam geometry); aperture-grille displays (Sony Trinitron, Mitsubishi Diamondtron) instead have continuous vertical phosphor stripes with one or two thin horizontal damper-wire shadows rather than scanline-row gaps. The auditor must check the rendered gap geometry against the CRT type the preset claims — evaluating a Trinitron simulation using shadow-mask row-gap geometry is invalid. Barrel distortion is present but mild (under 3% on a typical consumer set) and is worst at the corners. **What to verify:** `scanlineStrength` at moderate value should produce gaps consistent with the claimed CRT type — dark bands between rows for shadow/slotted mask, or near-continuous vertical stripes with faint damper-wire lines for aperture grille (real phosphor stripes transmit ~10–15% ambient light in the gap region); `barrelDistortion` at 0.15–0.2 should match the geometry of a typical 14-inch consumer TV rather than an extreme fish-eye; `phosphorPersistence` must leave a trailing glow only on bright areas (the persistence is luminance-dependent, not applied uniformly); `bloom` should affect only highlights above roughly 70% luminance, not the whole frame.

---

### Tape & Dropouts : Video Artifacts

**Real-world artifact to check:** VHS head-switching noise and tape dropouts from a third-generation dub. The head-switching glitch is the single most recognisable VHS tell: a torn, noisy horizontal band 6–10 scanlines tall at the very bottom of the picture, caused by the video head changing during the vertical blanking interval that overruns into the active picture area. Tape dropouts appear as brief, disrupted horizontal streaks, typically 1–4 scanlines tall and 20–80 pixels wide, caused by physical loss of magnetic contact between the tape oxide and the head. The exact polarity of a dropout depends on FM carrier state, deck AGC behaviour, and tape generation: a common presentation is a bright flash at the moment of head/oxide contact loss followed by a dark recovery, but all-white streaks or dark streaks with a bright recovery edge are also authentic — the polarity varies and is not fixed. Dropouts should be evaluated on correct shape (brief and horizontal), duration, and irregular clustering, not on a single mandated bright-then-dark order. **What to verify:** `advancedHeadSwitching` must produce the noise band only at the bottom of the frame (not centre or top); the band's vertical extent should match a real head-switch (approximately 6–12 scanlines); `advancedDropouts` must produce brief horizontal streaks clustered irregularly across the frame — verify shape, duration, and clustering against an actual dropout in the cleared reference rather than enforcing one fixed polarity; `advancedChromaDelay` must shift colour right of luma edges, not left — this is the same physical mechanism (chroma subcarrier lagging luma) described in the Colour Signal section; apply the threshold established there and do not double-count.
